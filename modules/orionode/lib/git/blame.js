...
function getBlame(req, res) {
	...
	.then(function(repo) {
		...
		fileRelativePath = req.params["0"].split("/").slice(1).join("/");
	...
	.then(function(blame){
		...
		createhunkCommitArray(blamerepo,hunks).then(function(hunkCommitArray){
			...
			sendingBlamejason = {
				...
				"Location": "/gitapi/blame/HEAD" + fileDir + fileRelativePath,
				"Type" : "Blame"
			};
