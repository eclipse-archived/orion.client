...
try {
	...
	var isIndexIncludeNodeModules;
	var isIndexOfCurrentWorkSpaceExsit = true;
		...
		return fs.statAsync(filePath)
			.then(function (stats) {
				...
				if (stats.isDirectory()) {
					if (filename[0] === "." || (!isIndexIncludeNodeModules && filename === "node_modules")) {
	...
	function saveIndexToFile(index, indexDir, userId) {
		...
		return new Promise(function (fulfill, reject) {
			originalFs.writeFile(path.join(indexDir,userId)+".json", JSON.stringify(index), function (err) {
				...
				if(!isIndexOfCurrentWorkSpaceExsit){
					isIndexOfCurrentWorkSpaceExsit = true;
					that.postMessage({
						isIndexOfCurrentWorkSpaceExsit: isIndexOfCurrentWorkSpaceExsit,
					});
				}
				fulfill(index);
			});
	...
	this.onmessage = function (event) {
		switch(event.data.type){
			case "startIndex":
			    isIndexIncludeNodeModules = event.data.indexNodeModules;
			...
			case "workspaceDirChange":
				...
				isIndexOfCurrentWorkSpaceExsit = false;
