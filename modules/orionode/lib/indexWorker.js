...
try {
	 ...
	 * Git Page Related operations: (indexAfterAllDone)
	...
	var Indexer = function (indexDir) {
		this.updateIndexEntry = function (updatePackage, userworkspaceDir, userID) {
			...
			if(oldfilePath){
				// Rename operation
				this.indexAfterAllDone(500, userworkspaceDir, userID);
			}else if(newFilePath){
	...
	var getIndexer = exports.getIndexer = function (indexDir) {
	...
	var scheduleIndex = exports.scheduleIndex = function (defaultWorkdir, interval, indexDir, userId) {
		return Promise.resolve(defaultWorkdir).then(function (defaultWorkdir) {
			return waitForAWhile(interval)
				.then(function () {
					return indexing(defaultWorkdir, indexDir, userId)
						.then(function () {
							return scheduleIndex(defaultWorkdir, interval, indexDir, userId);
						}).catch(function () {
							return scheduleIndex(defaultWorkdir, interval, indexDir, userId);
						});
				});
	...
	this.onmessage = function (event) {
		...
		scheduleIndex(event.data.workspaceDir, event.data.inverval, event.data.indexDir, event.data.userId);
	}.bind(this);
