			...
			tab.addEventListener('dragenter', function(evt){
				...
				if(evt.target.nodeName === "LI"){
					if(evt.target.isSameNode(dragSrcEl.previousSibling)){
						// drag forward
						var replacedTab1 = ul.replaceChild(dragSrcEl, evt.target);
						ul.insertBefore(replacedTab1, dragSrcEl.nextSibling);
					}else{
						// drag back
						var replacedTab2 = ul.replaceChild(dragSrcEl, evt.target);
						ul.insertBefore(replacedTab2, dragSrcEl);
					}
				}
	  			return false;
			});
