...
module.exports = function(options) {
	...
	if (USE_WORKERS) {
		...
		indexWorker.postMessage({workspaceDir: options.workspaceDir, inverval:options.configParams["filename.indexing.interval"],indexDir:options.indexDir, userId:"anonymous", isElectron:options.configParams.isElectron});
	} else {
	...
	.get('*', function(req, res) {
		search(req.originalUrl, req.user.workspaceDir, req.contextPath, options.indexDir, req.user.username, options.configParams.isElectron)
