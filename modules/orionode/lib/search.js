...
module.exports = function(options) {
	...
	if (USE_WORKERS) {
		...
		indexWorker.postMessage({workspaceDir: options.workspaceDir, inverval:options.configParams["filename.indexing.interval"],indexDir:options.indexDir, userId:"anonymous", isElectron:options.configParams.isElectron, isMultipleUser:!options.configParams['orion.single.user']});
	} else {
	...
	.get('*', function(req, res) {
		search(req.originalUrl, req.user.workspaceDir, req.contextPath, options.indexDir, req.user.username, options.configParams.isElectron, !options.configParams['orion.single.user'])
