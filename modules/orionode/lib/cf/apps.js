...
function getapps(req, res){
	...
	target.computeTarget(req.user.username, targetRequest)
	 .then(function(target){
		...
		if(encodeName){
			getAppwithAppName(req.user.username, task,encodeName);
			...
			if(manifestLocation){
				...
				.then(function(manifest){
					if(manifest && manifest.applications &&  manifest.applications[0]){
						return getAppwithAppName(req.user.username, task, manifest.applications[0].name);
					}			
				});
			}
		}else{
...
function getAppwithAppName(userId, task,encodeName){
	return _getAppwithAppName(userId, encodeName)
	.then(function(result){
		respondAppgetRequest(result,task);
	});
}
function _getAppwithAppName(userId, encodeName){
...
//		Promise.all(getRouteGuidbyGuid(req.user.username, routeGuid,task),getAppbyGuid(req.user.username, appGuid,task))
//		.then(function(resultArray){
//			return mapRoute(req.user.username, resultArray[0],task);
//		});
//	}
...
//	var packagerName = req.body.Packager; // TODO in java code, there is acually only one zip approach regardless this value.
	...
	return target.computeTarget(req.user.username, targetRequest)
	 .then(function(target){
		...
		return _getAppwithAppName(req.user.username, appName? appName : manifestAppName)
		...
		if(app){
			var waitFor;
			if(state === 'Started'){
				waitFor = startApp(req.user.username, userTimeout);
			}else if(state === 'Stopped'){
				waitFor = stopApp(req.user.username);
			}else{
		...
		return _getAppwithAppName(req.user.username, appCache.appName);
	}).then(function(){
		if(restart){
			return restartApp(req.user.username);
		}
		return startApp(req.user.username,-1);
...
function startApp(userId, userTimeout){
	var DEFAULT_TIMEOUT = 60;
	var MAX_TIMEOUT = 180;
	var cloudAccessToken = target.getAccessToken(userId);
...
function restartApp(userId){
	return stopApp(userId)
	.then(function(){
		return startApp(userId, -1);
	});
}
...
function createApp(req, res){
	...
	if(stack){
		waitForStackGuid = getStackGuidByName(req.user.username, stack);
	}
...
function getStackGuidByName(userId, stackname){
	var cloudAccessToken = target.getAccessToken(userId);
	return new Promise(function(fulfill) {
...
function bindRoute(req, res){
	var targetRequest = req.body.Target;
	var cloudAccessToken = target.getAccessToken(req.user.username);
	return domains.getCFdomains(req.user.username, targetRequest.Url, targetRequest.Org)
					...
					async.each(manifestService, function(service, cb) {
						return getServiceGuid(req.user.username, service)
							.then(function(serviceInstanceGUID){
								...
								return createService(req.user.username, service.label, servicePlanGuid)
...
function getServiceGuid(userId, service){
	return new Promise(function(fulfill) {
		var cloudAccessToken = target.getAccessToken(userId);
...
function createService(userId, serviceName, servicePlanGuid){
	var cloudAccessToken = target.getAccessToken(userId);
	var body = {
...
function bindService(userId, serviceGuid){
	var cloudAccessToken = target.getAccessToken(userId);
	var body = {
...
function getAppbyGuid(userId, appGuid, task){
...
function getRouteGuidbyGuid(userId, routeGuid, task){
	var cloudAccessToken = target.getAccessToken(userId, task);
	return new Promise(function(fulfill) {
...
function mapRoute(userId, routeGuid, task){
	var cloudAccessToken = target.getAccessToken(userId, task);
	return new Promise(function(fulfill) {
		var attachRouteHeader = {
