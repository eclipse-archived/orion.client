...
module.exports.router = function(options) {
	...
	function _getAppwithAppName(targetRequest,encodeName){
		...
		return new Promise(function(fulfill) {
			...
			return new Promise(function(fulfill) {
				var appsHeader = {
					url: targetRequest.Url + result.resources[0].metadata.url +'/summary',
					headers: {'Authorization': cloudAccessToken}
				};
				request(appsHeader, function (error, response, body) {
				...
				return new Promise(function(fulfill) {
					var appsHeader = {
						url: targetRequest.Url + result.resources[0].metadata.url +'/instances',
						headers: {'Authorization': cloudAccessToken}
					};
					request(appsHeader, function (error, response, body) {
...
//			packager = getDeploymentPackager(packagerName);
			...
			_getAppwithAppName(targetRequest,appCache.appName)
			.then(function(appResult){
				console.log("Get APP again");
				return appResult;
			});
			...
			if(restart){
				...
				return startApp(targetRequest,app,userTimeout,-1);
			}
		});
	}
