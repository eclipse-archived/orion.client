			...
			return new Promise(function(fulfill) {
				...
				waitForRoute.then(function(appRoute){
					...
					request.put(attachrouteHeader, function (error, response, body) {
						if(!error) return target.parsebody(body);
					});
				});
			});
		});
		...
		.then(function(filePath){
			...
			return new Promise(function(fulfill) {
				var uploadBitsHeader = {
					url: targetRequest.Url + '/v2/apps/' + appCache.appGuid + "/bits?async=true",
					headers: {'Authorization': cloudAccessToken, 'Content-type': 'multipart/form-data'},
					formData :{
						'my_file': fs.createReadStream(archiveredFilePath)
					}
				};
				request.put(uploadBitsHeader, function (error, response, body) {
					if(!error) fulfill(target.parsebody(body));
				});
			});
		}).then(function(result){
