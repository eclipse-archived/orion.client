...
function getAppwithAppName(userId, task,encodeName, appTarget){
	...
	.then(function(result){
		respondAppgetRequest(result.appJson,task);
	});
}
function _getAppwithAppName(userId, encodeName, appTarget){
	...
	return new Promise(function(fulfill) {
		...
		return new Promise(function(fulfill) {
			var appsHeader = {
				url: appTarget.Url + app.appUrl +'/summary',
				headers: {'Authorization': cloudAccessToken}
			};
			request(appsHeader, function (error, response, body) {
				app.summaryJson = target.parsebody(body);
				fulfill();
			});
		}).then(function(){
			return new Promise(function(fulfill) {
				var appsHeader = {
					url: appTarget.Url + app.appUrl +'/instances',
					headers: {'Authorization': cloudAccessToken}
				};
				request(appsHeader, function (error, response, body) {
					app.instanceJson = target.parsebody(body);
					fulfill();
				});
			});
		}).then(function(){
