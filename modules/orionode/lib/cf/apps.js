	...
	target.computeTarget(req.user.username, targetRequest)
	 .then(function(appTarget){
		if(encodeName){
			...
			if(manifestLocation){
				manifests.retrieveManifestFile(req, manifestLocation)
...
function respondAppPutRequest(task,status){
	return new Promise(function(fulfill,reject){
		...
		if (status.error_code) {
			return reject({"code":400, "message": status.description,"bundleid":"org.eclipse.orion.server.core","data":status});
		}
		if(status === "RUNNING"){
			...
			return reject({"code":400, "message":"Status wrong"});
		}
		task.done({
			HttpCode: 200,
			Code: 0,
	...
	.then(function(filePath){
		...
		function promiseWhile(value) {
			return Promise.resolve(value).then(function(result) {
				return collectCFRespond(result)
				.then(function(result){
					if(result === "finished"){
		...
		function collectCFRespond(result){
			if(result.status !== "finished" && result.status !== "failure"){
				if(result.status === "failed"){
					return new Promise.reject({"code":400, "message":"Upload failed"});
				}
				if(result.attemptsLeft === 0){
					return new Promise.reject({"code":400, "message":"Upload timeout exceeded"});
				}
			}else if(result.status === "failure"){
				return new Promise.reject({"code":400, "message":"Failed to upload application bits"});
			}else if(result.status === "finished"){
				return Promise.resolve("finished");
			}else{			
				return new Promise(function(fulfill){
					setTimeout(function(){
						return target.cfRequest("GET", req.user.username, appTarget.Url + result.metadata.url)
						.then(function(result){
							fulfill({
