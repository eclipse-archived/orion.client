...
function getService(req, res){
	...
	new Promise(function(fulfill) {
		...
		var fullService = [];
		function promiseWhile(predicate, value) {
		    return Promise.resolve(value).then(predicate).then(function(result) {
		    	return collectServiceInfo(result)
		    	.then(function(){
		    		if(result.next_url){
						serviceUrl = result.next_url;				
			            return promiseWhile(predicate, serviceUrl);
					}
					return null;
		    	});
		    });
		}
		function predicate(serviceUrl){
			var servicesHeader = {
			...
			return new Promise(function(fulfill , reject){
				async.each(serviceResources, function(resource, cb) {
					...
					if(resource.entity.service_plan){
						...
						return getCFService(targetURL,serviceGuid)
...
function getCFService(targetURL,serviceGuid){
	var cloudAccessToken = target.getAccessToken();
	return new Promise(function(fulfill) {
		var serviceHeader = {
