...
function getDomain(req, res){
	...
	getCFdomains(targetRequest, domainName,defaultDomainMode)		
...
function getCFdomains(targetRequest,domainName,defaultDomainMode){
	var cloudAccessToken = target.getAccessToken();
	var domainArray = [];
	return new Promise(function(fulfill) {
		if(defaultDomainMode){
			var privateDomainURL = orgs.getOrg(targetRequest.Org).entity.private_domains_url;
			var domainHeader = {
				url: targetRequest.Url + privateDomainURL,
			...
			domainHeader = {
				url: targetRequest.Url + '/v2/shared_domains',
				headers: {'Authorization': cloudAccessToken},
