...
function getCFdomains(appTarget, UserId, targetUrl, orgName, domainName, defaultDomainMode) {
	...
	return new Promise(function(fulfill) {
		if (!defaultDomainMode) {
			return cfutil.cfRequest('GET', UserId, null, targetUrl + appTarget.Org.entity.private_domains_url, 
		...
		return new Promise(function(fulfill) {
			if (defaultDomainMode || !domainName || result.total_results < 1) {
				...
				return cfutil.cfRequest('GET', UserId, null, targetUrl + '/v2/shared_domains' + '/v2/shared_domains', qs)
				.then(function(result){
					fulfill(result);
				});
			}
