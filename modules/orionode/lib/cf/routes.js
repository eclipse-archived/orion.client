...
module.exports.router = function() {
	...
	function getroutes(req, res) {
		...
		if (RouteJsonRequest) {
			if (globalCheck && globalCheck === "true") {
				...
				waitForGetRouteJson = checkRouteRequest(targetRequest, RouteJsonRequest.DomainName, RouteJsonRequest.Host);
			}
			waitForGetRouteJson = getRouteRequest(targetRequest, RouteJsonRequest.DomainName, RouteJsonRequest.Host);
		}
		waitForGetRouteJson = getRouteRequest(targetRequest, false);
	...
	function checkRouteRequest(targetRequest, domainName, host) {
		var cloudAccessToken = target.getAccessToken();
		return domains.getCFdomains(targetRequest.Url, targetRequest.Org, domainName)
	...
	function getRouteRequest(targetRequest) {
		var cloudAccessToken = target.getAccessToken();
		var routesArray = [];
		if (arguments.length === 3) {
			var domainName = arguments[1];
			var host = arguments[2];
		} else if (arguments.length === 2) {
			var orphaned = arguments[1];
		}
		return new Promise(function(fulfill) {
				if (domainName && host) {
					return domains.getCFdomains(targetRequest.Url, targetRequest.Org, domainName)
