...
try {
	...
	var FORCE_USING_INDEX_DEBUG = null; // For debug purpose, assgin true when you want force using index, or nothing by default ;
	...
	function search(originalUrl, workspaceDir, contextPath, userId, indexDir, isElectron, usingIndex, isIndexForCurrentWorkspaceExsit, excludeNames) {
	...
	this.onmessage = function(event) {
		search(event.data.originalUrl, event.data.workspaceDir, event.data.contextPath, event.data.userId, event.data.indexDir, event.data.isElectron, 
		event.data.usingIndex, event.data.isIndexForCurrentWorkspaceExsit, event.data.excludeNames)
