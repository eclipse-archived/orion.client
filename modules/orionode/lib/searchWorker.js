...
try {
	...
	function SearchOptions(originalUrl, contextPath, isMultipleUser, workspaceDir){
		...
		this.usingIndex =prefs.readPrefs(workspaceDir,isMultipleUser)
		.user.general.settings.generalSettings.filenameSearchPolicy;
		this.buildSearchOptions = function() {
			...
			for (var i = 0; i < terms.length; i++) {
				...
				if (isSearchField(term)) {
					...
					} else if(term.lastIndexOf("UsingIndex:", 0) === 0){
						this.usingIndex = true;
						// TODO need to see how to turn it off
					}
				} else {
	...
	function search(originalUrl, workspaceDir, contextPath, indexDir, userId, isElectron, isMultipleUser) {
		var searchOpt = new SearchOptions(originalUrl, contextPath, isMultipleUser, workspaceDir);
	...
	this.onmessage = function(event) {
		search(event.data.originalUrl, event.data.workspaceDir, event.data.contextPath, event.data.indexDir, event.data.userId, event.data.isElectron, event.data.isMultipleUser)
