...
function addNewTab(id, iframe) {
	...
	closeBt.addEventListener("click", function(evt) {
		if(tab.classList.contains("transient")){
			transientTab = null;
		}
...
function createTab(url, forceNew) {
	if(!forceNew){
		...
		var potentialExsitingIframe = Array.prototype.find.call(iframes,function(iframe){
			return iframe.contentWindow.location.href === url;
		});
	}
	if(!forceNew && potentialExsitingIframe){
		clickTab(potentialExsitingIframe.id.substr(6));
	}else if(!forceNew && transientTab){
		var correspondingiframeId = 'iframe' + transientTab.id.substr(3);
		var corrspondingiframe = document.querySelector('#'+correspondingiframeId);
		corrspondingiframe.src = url;
	}else{
		...
		iframe.addEventListener("load", function() {
			...
			electron.makeTabPermanent = makeTabPermanent;
			electron.createTab = createTab;
		...
		if(srcUrl.pathname === "/" || srcUrl.pathname.endsWith(".html")){
			...
			if(!transientTab){
				transientTab = newTab;
				newTab.classList.add("transient")
			}
		}else{
...
function otherEditorTabsHref(){
	var iframes = document.querySelectorAll(".tabContent");
	var editorTabsInfo = [];
	Array.prototype.forEach.call(iframes,function(iframe){
		var href = iframe.contentWindow.location.href.replace(/http:\/\/localhost:\w+\//, "");
		if(href.indexOf("/edit/edit.html")){
			editorTabsInfo.push({
				id : iframe.id.substr(6),
				href : iframe.contentWindow.location.href.replace(/http:\/\/localhost:\w+\/edit\/edit.html\#/, "")
			});
		}
	});
	return editorTabsInfo;
}
...
function makeTabPermanent(iframeId){
	if(iframeId){
		var correspondingTabId = 'tab' + iframeId.substr(6);
		var activeTransientTab = document.querySelector('#'+correspondingTabId);
		activeTransientTab.classList.remove("transient");
		transientTab = null;
	}else{
		transientTab.classList.remove("transient");
		transientTab = null;
	}
}
