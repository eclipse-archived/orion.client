...
function startServer(options) {
	...
	try {
		...
		if (options.configParams["orion.single.user"]) {
			...
			app.use(require(options.configParams["login.module"] || "./login/user")(options));
		}
		...
		var preAppendStaticAssets = options.configParams["pre.append.static.assets"] && options.configParams["pre.append.static.assets"].split(",") || [];
		var appendStaticAssets = options.configParams["append.static.assets"] && options.configParams["append.static.assets"].split(",") || [];
		preAppendStaticAssets.unshift('./modules/orionode/lib/orionode.client/');
		app.use(require('./lib/orion_static')({ orionClientRoot: ORION_CLIENT, maxAge: options.maxAge, preAppendStaticAssets: preAppendStaticAssets, appendStaticAssets: appendStaticAssets}));
