...
function(messages, mUndoStack, mKeyBinding, mRulers, mAnnotations, mTextDND, mRegex) {
	...
	TextActions.prototype = {
			...
			this._incrementalFindListener = {
				onVerify: function(e){
					...
					if (match && match.length > 0) {
						...
						self.editor.reportStatus(_formatMessage(messages.incrementalFind, prefix));
						...
						if (result) {
							...
							editor.reportStatus(_formatMessage(messages.incrementalFindNotFound, prefix), "error");
				...
				if (!this._incrementalFindActive) {
					...
					if (prefix.length !== 0) {
						...
						if (result) {
							...
							editor.moveSelection(result.index, result.index + result.length);
							this._incrementalFindIgnoreSelection = false;
							editor.reportStatus(_formatMessage(messages.incrementalFind, prefix));
						} else {
							editor.reportStatus(_formatMessage(messages.incrementalFindNotFound, prefix), "error");
		...
		toggleIncrementalFind: function() {
			...
			if (this._incrementalFindActive) {
				this.editor.reportStatus(_formatMessage(messages.incrementalFind, this._incrementalFindPrefix));
