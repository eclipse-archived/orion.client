...
], function(messages, mTextView, mProjectionTextModel, textUtil, Markdown, util) {
						...
						if (data) {
							...
							switch(data.type) {
								case 'markdown': {
									...
									sectionDiv.innerHTML = (data.title ? Markdown(data.title) : tipSection.title);
									tooltipContents.appendChild(sectionDiv);
									divResult = util.createElement(tooltipDoc, "div"); //$NON-NLS-0$
									divResult.innerHTML = Markdown(data.hover);
