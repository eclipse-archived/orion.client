...
define("orion/editor/editor", [ //$NON-NLS-0$
	...
	objects.mixin(Editor.prototype, /** @lends orion.editor.Editor.prototype */ {
		...
		install : function() {
			...
			if (this._annotationFactory) {
				...
				if (ruler) {
					...
					ruler.addAnnotationType(AT.ANNOTATION_DIFF_ADDITION);
					ruler.addAnnotationType(AT.ANNOTATION_DIFF_DELETION);
			...
			if (this._lineNumberRulerFactory) {
		        ...
		        this._lineNumberRuler.addAnnotationType(AT.ANNOTATION_DIFF_ADDITION);
		        this._lineNumberRuler.addAnnotationType(AT.ANNOTATION_DIFF_MODIFIED);
		        this._lineNumberRuler.addAnnotationType(AT.ANNOTATION_DIFF_DELETION);
		...
		showDiffAnnotations: function(diffs) {		
			...
			var remove = [], add = [];
			var model = annotationModel.getTextModel();
			var annotations = annotationModel.getAnnotations(0, model.getCharCount());
			var annotation;

			// Remove existing diff annotations
			while (annotations.hasNext()) {
				annotation = annotations.next();
				if (annotation.type === AT.ANNOTATION_DIFF_ADDITION ||
					annotation.type === AT.ANNOTATION_DIFF_DELETION ||
					annotation.type === AT.ANNOTATION_DIFF_MODIFIED) {
					remove.push(annotation);
				}
			}

			// Add an annotation for the specified property between the specified start and end ranges
			if (diffs.additions) {
				diffs.additions.forEach(function(addition) {
					var start = model.getLineStart(addition.start);
					// If the closing line number of the addition range is on the last line,
					// get the line ending offset of the previous line
					var end = model.getLineCount() === addition.end
								? model.getLineEnd(addition.end - 1)
								: model.getLineStart(addition.end);
					annotation = mAnnotations.AnnotationType.createAnnotation(AT.ANNOTATION_DIFF_ADDITION,
						start, end, ""); //$NON-NLS-0$
					add.push(annotation);
				});
			}
			...
			if (diffs.modifications) {
				diffs.modifications.forEach(function(modified) {
					...
					annotation = mAnnotations.AnnotationType.createAnnotation(AT.ANNOTATION_DIFF_MODIFIED,
						start, end, ""); //$NON-NLS-0$
					add.push(annotation);
				});
			}
			...
			if (diffs.deletions) {
				diffs.deletions.forEach(function(deletion) {
					var line = model.getLineStart(deletion);
					annotation = mAnnotations.AnnotationType.createAnnotation(AT.ANNOTATION_DIFF_DELETION,
						line, line+1, ""); //$NON-NLS-0$
					add.push(annotation);
				});
			}	
			annotationModel.replaceAnnotations(remove, add);
