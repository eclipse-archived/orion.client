	...
	objects.mixin(Editor.prototype, /** @lends orion.editor.Editor.prototype */ {
		...
		showDiffAnnotations: function(diffs) {
			...
			for (var i = 0; i < diffs.added.length; i++){
				totalDiff.push({
					lineStart: diffs.added[i].lineStart,
					lineEnd: diffs.added[i].lineEnd,
					type:AT.ANNOTATION_DIFF_ADDED
				});
			}
			for (var i = 0; i < diffs.modified.length; i++){
				totalDiff.push({
					lineStart: diffs.modified[i].lineStart,
					lineEnd: diffs.modified[i].lineEnd,
					type:AT.ANNOTATION_DIFF_MODIFIED
				});
			}
			for (var i = 0; i < diffs.deleted.length; i++){
				totalDiff.push({
					lineStart: diffs.deleted[i].lineStart,
					lineEnd: diffs.deleted[i].lineEnd,
					type:AT.ANNOTATION_DIFF_DELETED
				});
			}
			
			this.showAnnotations(totalDiff, [
