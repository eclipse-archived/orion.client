	...
	function GitCommitListExplorer(options) {
		...
		mGitCommands.getModelEventDispatcher().addEventListener("modelChanged", this._modelListener = function(event) { //$NON-NLS-0$
			...
			case "checkoutFile": //$NON-NLS-0$
				...
				Deferred.when(that.model.root.repository.status || (that.model.root.repository.status = that.progressService.progress(that.gitClient.getGitStatus(theLocation), messages["Getting changes"])),  function(theStatus) {
