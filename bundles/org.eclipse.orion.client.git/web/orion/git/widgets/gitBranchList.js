	...
	objects.mixin(GitBranchListModel.prototype, /** @lends orion.git.GitBranchListModel.prototype */ {
		...
		getChildren: function(parentItem, onComplete){
			...
			if (parentItem.children && !parentItem.more) {
				...
				var deferred = that.progressService.progress(that.gitClient.doPullRequestList( repository.PullRequestLocation), msg);
				deferred.then(function(resp){
					...
					if (jsonData.JsonData.GitHubAuth) {
						var credentialsDialog = new mGitCredentials.GitCredentialsDialog({
							title: messages["Authorize to get pull requests"],
							serviceRegistry: that.registry,
							func: func,
							errordata: jsonData.JsonData,
							failedOperation: jsonData.failedOperation,
							closeCallback : function(){
								that.handleError(jsonData);
								onComplete([]);
							},
							onlyAuthorizeWithGitHub:true
						});
