		...
		getChildren: function(parentItem, onComplete){
			...
			if (parentItem.children && !parentItem.more) {
				...
				mGitCommands.getDefaultSshOptions(that.registry, repository).then(function(options){
						...
						var progress = that.registry.getService("orion.page.progress"); //$NON-NLS-0$
						var gitService = that.registry.getService("orion.git.provider"); //$NON-NLS-0$
						var progressService = that.registry.getService("orion.page.message"); //$NON-NLS-0$
						var deferred = progress.progress(gitService.doPullRequestList(repository.PullRequestLocation, repository.GitUrl,
								options.gitSshUsername, options.gitSshPassword, options.knownHosts, options.gitPrivateKey,
								options.gitPassphrase), msg);
						progressService.createProgressMonitor(deferred, msg);
