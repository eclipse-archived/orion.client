...
define([
	...
	function getCommitSvgs(commits) {
		...
		var updateDrawingForCommit = function(drawingUpdate, svg) {
			if (drawingUpdate) {
				var x1 =(COLUMN_SPACING) * (drawingUpdate.fromColumn + 1),x2;
				if(drawingUpdate.fromColumn>drawingUpdate.toColumn){
					x2 = (COLUMN_SPACING) * (drawingUpdate.toColumn + 1) - LINE_WIDTH;
				}else if(drawingUpdate.fromColumn<drawingUpdate.toColumn){
					x2 = (COLUMN_SPACING) * (drawingUpdate.toColumn + 1) + LINE_WIDTH;
				}else{
					x2 = (COLUMN_SPACING) * (drawingUpdate.toColumn + 1);
				}
