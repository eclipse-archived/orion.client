...
define([
	...
	function createFilter(section, msg, callback) {
		...
		button.commandTooltip = createTooltip(button);
		...
		button.addEventListener("click", this._filter = function(){ //$NON-NLS-0$
			doFilter();
		});
		...
		filter.addEventListener("keydown", this._keyDownFilter = function(e) { //$NON-NLS-0$
			if (e.keyCode === 13) {
				doFilter();
			}
		});
		...
		return {
			...
			destroy: function() {
				this.button.removeEventListener("click", this._filter);
				this.button.commandTooltip.destroy();
