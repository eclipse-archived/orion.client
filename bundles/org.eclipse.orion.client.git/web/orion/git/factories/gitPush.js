...
define(['i18n!git/nls/gitmessages','orion/commandRegistry','orion/git/widgets/ConfirmPushDialog','orion/git/gitPreferenceStorage','orion/git/factories/gitCommon','orion/Deferred'
	 ...
	 * @param explorer
	 * @param toolbarId
	 * @param refreshExplorer If equal to true then commands interacting with the UI will call explorer.changedItem
	 */
	return function(serviceRegistry, commandService, explorer, toolbarId, refreshExplorer) {
		...
		var perform = function(data) {
					...
					deferred.then(
						function(jsonData){
							handleGitServiceResponse(jsonData, serviceRegistry, 
								function() {
									if (refreshExplorer) explorer.changedItem();
								}, function (jsonData) {
				...
				progress.progress(gitService.getGitClone(item.CloneLocation), "Getting git repository details " + item.Name).then(
					function(clone){
						...
						gatherSshCredentials(serviceRegistry, commandInvocation).then(
							function(options) {
