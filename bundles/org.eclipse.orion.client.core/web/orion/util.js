...
define(['dojo', 'dijit', 'dojo/hash', 'dijit/form/ValidationTextBox'], function(dojo, dijit) {
	 ...
	 */
	function addNavGrid(rowModel, domNode, widget, onClick) {
		var navHolder = getNavGridHolder(rowModel);
		if(navHolder){
	...
	/**
	 * Get the grid navigation holder from a row navigation model. A row navigation model normally comes from any node in a {treeModelIterator}.
	 * If not exist yet, the .gridChildren property will be lazily created on the row model.
	 *
	 * @param {object} rowModel the row model from the {treeModelIterator}.
	 * @returns {Array} The .gridChildren property of a row model.
	 */
	function getNavGridHolder(rowModel) {
		if(!rowModel){
			return null;
		}
		if(!rowModel.gridChildren){
			rowModel.gridChildren = [];
		}
		return rowModel.gridChildren;
	}
