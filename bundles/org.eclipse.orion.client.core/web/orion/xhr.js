...
define(['orion/Deferred'], function(Deferred) {
	...
	var XSRF_NONCE_SESSION_KEY = "x-csrf-token";

	function addNonceRequest(method, headers) {
		if(typeof sessionStorage[XSRF_NONCE_SESSION_KEY] === 'undefined') {
			if(method.match("GET")) {
				headers[XSRF_NONCE_REQUEST_ATTR] = XSRF_NONCE_FETCH;
			}
		} else {
			headers[XSRF_NONCE_REQUEST_ATTR] = sessionStorage[XSRF_NONCE_SESSION_KEY];
		}
	}

	function extractNonce(responsevalue) {
		if(responsevalue != "" && responsevalue != null) {
			sessionStorage[XSRF_NONCE_SESSION_KEY] = responsevalue;
		}
	}
