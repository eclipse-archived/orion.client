...
patterns:[{include:"orion.patterns"},{match:"\\b(?:"+n.join("|")+")\\b",name:"keyword.control.js"},{begin:"'(?:\\\\.|[^\\\\'])*\\\\$",end:"^(?:$|(?:\\\\.|[^\\\\'])*('|[^\\\\]$))",name:"string.quoted.single.js"},{begin:'"(?:\\\\.|[^\\\\"])*\\\\$',end:'^(?:$|(?:\\\\.|[^\\\\"])*("|[^\\\\]$))',name:"string.quoted.double.js"}]});return{id:g[g.length-1].id,grammars:g,keywords:n}});
...
captures:{2:{name:"keyword.other.documentation.task"},4:{name:"comment.line"}}}]},{begin:"</?[A-Za-z0-9]+",end:"/?>",captures:{"0":{name:"entity.name.tag.html"}},name:"meta.tag.html",patterns:[{include:"#comment"},{include:"orion.patterns#string"},{include:"orion.patterns#string_singleQuote"}]}]});return{id:g[g.length-1].id,grammars:g,keywords:[]}});
...
e;this._properties["service.names"]=e;this.service=b}function f(a,e,c){if(!a[e])throw e+g[" is not supported in this file system"];return a[e].apply(a,c)}function e(d,e){function c(a){var b=new n;b.reject(g["No Matching FileService for location:"]+a);return b}var b=[];d.forEach(function(c){var d=new a(c.impl);b.push(new l(c.id,"readonly.file.service",c.serviceProperties,d))});var f=b;if(e)for(var f=[],m=0;m<b.length;++m)e(b[m])&&f.push(b[m]);for(var r=[],x=[],u=[],t=[],y={fetchChildren:function(){var a=
...
{Directory:!0,Length:0,LocalTimeStamp:0,Location:f[m].getProperty("top"),ChildrenLocation:f[m].getProperty("top"),Name:f[m].getProperty("Name")};var s=f[m].getProperty("pattern")||".*";"^"!==s[0]&&(s="^"+s);r[m]=RegExp(s);x[m]=f[m].service;u[m]=f[m].getProperty("Name");f[m].getProperty("NameKey")&&f[m].getProperty("nls")&&p.getMessageBundle(f[m].getProperty("nls")).then(function(a,b){t[a].Name=b[f[a].getProperty("NameKey")];u[a]=b[f[a].getProperty("NameKey")]}.bind(this,m))}this._getServiceIndex=
function(a){if("/"===a)return-1;if(!a||a.length&&0===a.length)return x[0]?0:-1;for(var b=0;b<r.length;++b)if(r[b].test(a))return b;throw g["No Matching FileService for location:"]+a;};this._getService=function(a){a=this._getServiceIndex(a);return-1===a?y:x[a]};this._getServiceName=function(a){a=this._getServiceIndex(a);return-1===a?y.Name:u[a]};this._getServiceRootURL=function(a){a=this._getServiceIndex(a);return-1===a?y.Location:t[a].Location}}l.prototype={getPropertyKeys:function(){var a=[],e;for(e in this._properties)this._properties.hasOwnProperty(e)&&
...
n,p,a,l,f,e,d,h,c,b,k,m,r,x,u,t,y){function s(a){this._parentDomNode=y.node(a.parent);this._parentDomNode.classList.add("browserParentDome");this._fileClient=a.fileClient;this._repoURL=a.repoURL;a.serviceRegistry?this._fileClient=new c.FileClient(a.serviceRegistry):!this._fileClient&&a.serviceRefs&&(this._fileClient=new m.FileClient(a.serviceRefs));this._fileClient||(this._fileClient=new r.FileClient);this._syntaxHighlighter=a.syntaxHighlighter;this._syntaxHighlighter||(this._syntaxHighlighter=new k.SyntaxHighlighter);
this._contentTypeService=a.contentTypeService;this._contentTypeService||(this._contentTypeService=new b.ContentTypeRegistry(k.ContentTypes));this._preferences=a.preferences;this.rootName=a.rootName;this._showBranch=a.showBranch;this._showComponent=a.showComponent;this._init(a)}t.mixin(s.prototype,{_init:function(a){this._commandRegistry=new h.CommandRegistry({});this._maxEditorLines=a.maxEditorLines;this._inputManager=new n.InputManager({fileClient:this._fileClient,statusReporter:this._statusReport,
contentTypeRegistry:this._contentTypeService});this._uriTemplate=new u("#{,resource,params*}");this._inputManager.addEventListener("InputChanged",function(a){var b=a.metadata;if(this._branches&&this._branchSelector){var c=this._branches[0].Name;this._activeBranchLocation=this._branches[0].Location;var d=null,e=!1;b.Parents?(0<b.Parents.length?(c=b.Parents[b.Parents.length-1].Name,this._activeBranchLocation=b.Parents[b.Parents.length-1].Location):(c=b.Name,this._activeBranchLocation=b.Location),e=
...
branches:a}));this.refresh(g.hash())}.bind(this))}else this.refresh(g.hash())},_breadCrumbMaker:function(a,b){this._renderBreadCrumb({task:"Browse",name:this._breadCrumbName,target:this._breadCrumbTarget,breadCrumbContainer:a,makeBreadcrumbLink:function(a,b,c){this._makeBreadCrumbLink(a,b,c)}.bind(this),makeBreadcrumFinalLink:!1,fileClient:this._fileClient,maxLength:b})},_setEditor:function(a){this._editor!==a&&(this._editor=a)},_statusReport:function(){},_makeBreadCrumbLink:function(a,b,c){(c=c?
...
c.fileServiceRootURL(e.Location):null;new p.BreadCrumbs({container:d,maxLength:a.maxLength,resource:e,rootSegmentName:b,workspaceRootSegmentName:this.rootName?this.rootName:c,workspaceRootURL:this._activeBranchLocation,makeFinalHref:a.makeBreadcrumFinalLink,makeHref:a.makeBreadcrumbLink})}},_getEditorView:function(b,c){var d=null;if(c&&b){var e={parent:this._parentDomNode,readonly:!0,showProjectView:!1,maxEditorLines:this._maxEditorLines,readmeHeaderClass:"readmeHeader",metadata:c,branchSelector:this._branchSelector,
commandRegistry:this._commandRegistry,contentTypeRegistry:this._contentTypeService,inputManager:this._inputManager,fileService:this._fileClient,breadCrumbMaker:function(a,b){this._breadCrumbMaker(a,b)}.bind(this)};if(c.Directory)d=new a.BrowseView(e);else{var f=b.editor;if(!f||"orion.editor"===f){var g=this._contentTypeService.getFileContentType(c);"text/x-markdown"===g.id?e.isMarkdownView=!0:l.isImage(g)?(e.imageView={},this._fileClient.readBlob(c.Location).then(function(a){var a=URL.createObjectURL(new Blob([a],
{type:g.id})),b=document.createElement("img");b.src=a;b.classList.add("readonlyImage");this._currentEditorView.updateImage(b)}.bind(this))):e.editorView=this._editorView;d=new a.BrowseView(e)}}}this._currentEditorView!==d&&(this._currentEditorView&&this._currentEditorView.destroy(),(this._currentEditorView=d)&&this._currentEditorView.create());return this._currentEditorView},refresh:function(a){a||(a=this._fileClient.fileServiceRootURL(""));this._inputManager.setInput(a)},create:function(){},destroy:function(){}});
return{FileBrowser:s}});
...
method:"progress",params:Array.prototype.slice.call(arguments)})})):(e.result=g,l(e)))}catch(h){e&&(e.error=f(h),l(e))}}function y(a){try{if(a.method){var b=a.method,c=a.params||[];if("serviceId"in a){var d=Y[a.serviceId];d||u(a.id,"service not found");b in d?t(a.id,d,d[b],c):u(a.id,"method not found")}else if("objectId"in a){var e=T[a.objectId];e||u(a.id,"object not found");b in e?t(a.id,e,e[b],c):u(a.id,"method not found")}else if("requestId"in a){var f=R[a.requestId];f&&("cancel"===b&&f.cancel)&&
...
var f=new d(e,["handleEvent"]),g=b.addEventListener;b.addEventListener=function(a,b){c._namedListeners[a]||g(a,f);c.addEventListener(a,b)};var k=b.removeEventListener;b.removeEventListener=function(a,b){c.removeEventListener(a,b);c._namedListeners[a]||k(a,f)}}e=s(a);e=h.registerService(a.names||a.type,b,e);D[a.serviceId]={registration:e,proxy:b}}function z(){h.persist(a,{created:w,headers:B,services:A,autostart:C,lastModified:M})}var q=this,c=c||{},w=c.created||(new Date).getTime(),B=c.headers||{},
A=c.services||[],C=c.autostart,M=c.lastModified||0,E="installed",N,I,H,J,D={},O=0,P=0,R={},S={},T={},Y={},U;this._persist=z;this._resolve=function(){E="resolved";h.dispatchEvent(new e("resolved",q))};this._getAutostart=function(){return C};this._getCreated=function(){return w};this.getLocation=function(){return a};this.getHeaders=function(){return JSON.parse(JSON.stringify(B))};this.getName=function(){var a=this.getHeaders();return a?a.name||"":null};this.getVersion=function(){var a=this.getHeaders();
...
function(c){if("uninstalled"===E)return(new g).reject(Error("Plugin is uninstalled"));if(!c)return 0===M&&(M=(new Date).getTime(),z()),h.loadManifest(a).then(U);var d=B,f=A,k=C;B=c.headers||{};A=c.services||{};C=c.autostart||C;c.lastModified?M=c.lastModified:(M=(new Date).getTime(),z());if(p(B,d)&&p(A,f)&&C===k)return(new g).resolve();if("active"===E||"starting"===E){var l=[];Object.keys(A).forEach(function(a){var b=A[a];l.push(a);var c=D[a];if(c){if(p(b.methods,Object.keys(c.proxy))){var a=s(b),
...
this.installPlugin=function(b,c){var b=a(b),d=this.getPlugin(b);if(d)return(new g).resolve(d);if(w[b])return w[b];if(c)return d=new h(b,c,B),v.push(d),d._persist(),B.dispatchEvent(new e("installed",d)),(new g).resolve(d);var f=B.loadManifest(b).then(function(a){d=new h(b,a,B);v.push(d);d._persist();delete w[b];B.dispatchEvent(new e("installed",d));return d},function(a){delete w[b];throw a;});return w[b]=f};this.getPlugins=function(){return v.slice()};this.getPlugin=function(b){var c=null,b=a(b);v.some(function(a){if(b===
