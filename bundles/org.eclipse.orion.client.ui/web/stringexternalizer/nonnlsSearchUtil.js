...
define(['orion/Deferred'], function(Deferred) {
	...
	NonNlsSearch.prototype.parseDirectory = function(root) {
		...
		return def.then(function(jsonData) {
			return self.parseDirectoryData(jsonData);
		});
	};
	...
	NonNlsSearch.prototype.parseDirectoryData = function(jsonData) {
		...
		if (jsonData.Directory) {
			if (jsonData.Children) {
				...
				return Deferred.all(deferreds).then(function(results) {
					var ret = {
						Children: []
					};
					results.forEach(function(result) {
						ret.Children = ret.Children.concat(result.Children);
					});
					return ret;
				});
			} else {
				return this.parseDirectory(jsonData.ChildrenLocation);
			}
		} else if (jsonData.Name.lastIndexOf(".js") === (jsonData.Name.length - 3)) { //$NON-NLS-0$
