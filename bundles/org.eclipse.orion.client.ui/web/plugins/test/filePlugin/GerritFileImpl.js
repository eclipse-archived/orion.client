...
define(["orion/xhr", "orion/URITemplate", "orion/Deferred", "orion/URL-shim"], function(xhr, URITemplate, Deferred) {
	...
	function GerritFileImpl(pluginURL, project) {
		...
		this._listTemplateNoPath = new URITemplate(pluginURL + "/git/{project}/list{+ref}");
		this._contentTemplate = new URITemplate(pluginURL + "/git/{project}/contents/{+ref}/{+path}");
		this._contentTemplateNoPath = new URITemplate(pluginURL + "/git/{project}/contents/{+ref}");
	...
	GerritFileImpl.prototype = {
		...
		_getParents: function(location) {
			...
			var rootLocation = this._listTemplateNoPath.expand({project: project, ref: ref});
			...
			if (segments.length > 0) {
				...
				parentLocation = this._listTemplateNoPath.expand({project: project, ref: ref});
			}
		...
		fetchChildren: function(location) {
			...
			return xhr("GET", location, {
				...
				return directory.map(function(entry) {
					...
					var template;
					if (entry.type === "file") {
						template = entry.path ? _this._contentTemplate : _this._listTemplateNoPath;
					} else {
						template = entry.path ? _this._listTemplate : _this._listTemplateNoPath;
					}
