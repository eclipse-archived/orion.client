...
define([
	...
	'orion/i18nUtil',
	'orion/URL-shim'
], function(messages, sharedMessages, bookmarkMessage, Deferred, mExplorer, lib, mSearchCrawler, extensionCommands, mCommands, navigatorRenderer, objects, mSyntaxchecker,
	...
	function processTotalBookmarks(fileItem, bookmarksInFile, totalBookmarks) {
		bookmarksInFile.forEach(function(bookmark) {
			...
			bookmark.location = fileItem.location + bookmark.message + bookmark.lineStart + bookmark.lineStart;
	...
	function recalculateOffset(textContent, bookmarks) {
		...
		bookmarks.forEach(function(bookmark) {
			...
			bookmark.line = textModel.getLineAtOffset(bookmark.lineStart) + 1;
		});
		d1.resoleve();
		return d1;
	}
...
//				Object.keys(jsonData).forEach(loc) {
				
				for (var fileLocation in jsonData) {
					if (jsonData[fileLocation].length > 0) {
						var loc = fileLocation;
						d.push( (this.progressService ? this.progressService.progress(this.fileClient.read(loc), "Reading file " + loc) : this.fileClient.read(loc)).then(function(fileContent) {
							return recalculateOffset(fileContent, jsonData[loc]);
						}).then(function() {
							var loc = fileLocation;
							var filename = fileLocation.split("/").slice(-1)[0];
							var rootURL = this.fileClient.fileServiceRootURL(loc);
							var path = loc.substring(rootURL.length); //remove file service root from path
							var fileItem = {
								location: loc,
								path: path,
								type: "file",
