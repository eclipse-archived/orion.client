...
define([
	...
	function processTotalBookmarks(fileItem, bookmarksInFile, totalBookmarks) {
		bookmarksInFile.forEach(function(bookmark) {
			...
			bookmark.description = bookmark.message;
			bookmark.location = fileItem.location + bookmark.description + bookmark.lineStart + bookmark.lineStart;
	...
	function processBookmarksByName(totalBookmarks) {
		...
		var allbookmarks = {
			...
			name: bookmarkMessage["AllBookmarks"]
		}; //$NON-NLS-1$ //$NON-NLS-2$
