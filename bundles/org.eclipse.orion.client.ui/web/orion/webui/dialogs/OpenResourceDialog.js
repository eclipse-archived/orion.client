...
define([
		...
		this.$fileName.addEventListener("keydown",function(evt) { //$NON-NLS-0$
			if (evt.keyCode === lib.KEY.ENTER) {
				...
				if (link) {
					...
					self.saveOpenedFileName(link.resource);
				}
			}
		}, false);
	...
	OpenResourceDialog.prototype.updateTitle = function() {
		...
		this.showRecentSearchedOpenedFiles();
	};
	...
	OpenResourceDialog.prototype.decorateResult = function(resultsDiv) {
		...
		function clicked(evt) { //$NON-NLS-0$
			...
			self.saveOpenedFileName(evt.srcElement.resource);
		}
	...
	/** @private */
	OpenResourceDialog.prototype.saveOpenedFileName = function(resource) {
	...
	/** @private */
	OpenResourceDialog.prototype.showRecentSearchedOpenedFiles = function() {
		var exsitingSearchedOpenedFileList = JSON.parse(sessionStorage.getItem("lastSearchedOpendFiles"));
		if(exsitingSearchedOpenedFileList && exsitingSearchedOpenedFileList.length > 0 ){
			var renderFunction = this._searchRenderer.makeRenderFunction(this._contentTypeService, this.$results, messages["Recent Searched Opened Files"], this.decorateResult.bind(this));
