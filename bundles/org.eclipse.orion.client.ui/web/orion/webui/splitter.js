	...
	Splitter.prototype = /** @lends orion.webui.Splitter.prototype */ {
		...
		setOrientation: function(value) {
			...
			if (this._vertical) {
				...
				// KLUDGE !! we need to be able to get the real margin
				this._MarginHack = 3;
		...
		_adjustSplitterSize: function() {
			// Size in pixels
			var rect = lib.bounds(this.$splitter);
			this._spltterSize = rect[this._widthHeight] + this._MarginHack;
		...
		_resize: function() {
			...
			self._notifyResizeListeners(self.$trailing);
			self._notifyResizeListeners(self.$leading);
		},
		...
		_move: function(clientX, clientY) {
			...
			if (this._collapseTrailing) {
				...
				this._offset = this._adjustOffset(pos - (offset + this._MarginHack));
			}
