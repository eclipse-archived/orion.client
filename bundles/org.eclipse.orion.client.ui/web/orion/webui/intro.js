...
define(["orion/webui/littlelib",
	...
	Intro.prototype = /** @lends orion.webui.Intro.prototype */ {
		...
		registerTutorialNotification:function(tutorial){
			...
			var pulseMaker = document.createElement("div");
			var pulseRays = document.createElement("div");
			notification.classList.add("tutoNotification");
			pulseMaker.classList.add("pulse_marker");
			pulseRays.classList.add("pulse_rays");
			pulseMaker.appendChild(pulseRays);
			notification.appendChild(pulseMaker);
			...
			if(targetPlaceElement){
				...
				notification.style.top = rect.top + 8 + "px"; //$NON-NLS-0$
				notification.style.left = rect.left + 17 + "px"; //$NON-NLS-0$ 
			}
