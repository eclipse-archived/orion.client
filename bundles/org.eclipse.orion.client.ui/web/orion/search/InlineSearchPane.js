...
define([
	...
	objects.mixin(InlineSearchPane.prototype, /** @lends orion.search.InlineSearchPane.prototype */ {
	    ...
	    _initReplaceBox: function() {
			...
			var previewButtonSpan = document.createElement("span"); //$NON-NLS-0$
			previewButtonSpan.classList.add("core-sprite-search");
			this._replaceButton = this._replaceBox.getButton();
			this._replaceButton.classList.add("searchButton"); //$NON-NLS-0$
			this._replaceButton.appendChild(previewButtonSpan);
			previewButtonSpan.title = messages["Show replacement preview"]; //$NON-NLS-0$
		...
		_displaySelectedSearchScope: function(searchLocations) {
			...
			searchLocations.forEach(function(searchLocation){
				...
				locationElement.title = scopeString;
				scopeElementWrapper.title = scopeString;
