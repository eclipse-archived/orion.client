...
//			});
	        ...
	        switch (col_no) {
	            ...
	            case 1:
	                ...
	                if (item.type === "file") { //$NON-NLS-0$
	                    ...
	                    this.explorer.fileClient.read(item.location, true)
		                    .then(function(fileMeta){
	                   			this.generateBreadCrum(tableRow, fileMeta);
		                    }.bind(this));
	                } else if (item.type === "group") { //$NON-NLS-0$
	                	col.colSpan = 2;
	                	span = _createSpan(null, null, col, null);
	                    this.renderGroupElement(item, span, this.explorer.model);
	                    this.generateBreadCrum(tableRow, item.name);
	                } else {
	    ...
	    generateBreadCrum: function(tableRow, metadata){
