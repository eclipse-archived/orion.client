...
define(
		[ 'i18n!orion/nls/messages', 'require', 'orion/commonHTMLFragments',
				'orion/keyBinding', 'orion/commandRegistry', 'orion/commands',
				'orion/parameterCollectors', 'orion/extensionCommands',
				'orion/uiUtils', 'orion/keyBinding', 'orion/breadcrumbs',
				'orion/webui/littlelib', 'orion/webui/splitter',
				'orion/webui/dropdown', 'orion/webui/tooltip',
				'orion/favorites', 'orion/contentTypes', 'orion/URITemplate',
				'orion/PageUtil', 'orion/widgets/themes/ThemePreferences',
				'orion/widgets/themes/container/ThemeData', 'orion/Deferred',
				'orion/widgets/UserMenu', 'orion/PageLinks',
				'orion/webui/dialogs/OpenResourceDialog',
				'text!orion/banner/banner.html',
				'text!orion/banner/footer.html',
				'text!orion/banner/toolbar.html',
				'orion/widgets/input/DropDownMenu',
				'orion/widgets/input/GroupedContent', 'orion/util',
				'orion/customGlobalCommands'],
		function(messages, require, commonHTML, KeyBinding, mCommandRegistry,
				mCommands, mParameterCollectors, mExtensionCommands, mUIUtils,
				mKeyBinding, mBreadcrumbs, lib, mSplitter, mDropdown, mTooltip,
				mFavorites, mContentTypes, URITemplate, PageUtil,
				mThemePreferences, mThemeData, Deferred, mUserMenu, PageLinks,
				openResource, BannerTemplate, FooterTemplate, ToolbarTemplate,
				DropDownMenu, GroupedContent, util, mCustomGlobalCommands) {

			/**
			 * This class contains static utility methods. It is not intended to
			 * be instantiated.
			 * 
			 * @class This class contains static utility methods for creating
			 *        and managing global commands.
			...
			var notifyAuthenticationSite = qualifyURL(require
					.toUrl('auth/NotifyAuthentication.html')); //$NON-NLS-0$
			var authRendered = {};
			...
			function setUserIcon() {
				...
				userTriggerClassName = userTriggerClassName
						+ ' core-sprite-silhouette-inverse';
			...
			var customGlobalCommands = {
				...
				beforeGenerateRelatedLinks: mCustomGlobalCommands.beforeGenerateRelatedLinks || function(serviceRegistry, item, exclusions,
					commandRegistry, alternateItem){
				...
				addRelatedLinkCommand: mCustomGlobalCommands.addRelatedLinkCommand || function(command, invocation) {
						var relatedLinksNode = lib.node('relatedLinks');
						var newRelatedLinkItem = mCommands.createCommandMenuItem(
								relatedLinksNode, command, invocation);
						newRelatedLinkItem.classList.remove('dropdownMenuItem');
					},
				afterGenerateRelatedLinks: mCustomGlobalCommands.afterGenerateRelatedLinks || function(serviceRegistry, item, exclusions,
					commandRegistry, alternateItem) {},
				afterSetPageTarget: mCustomGlobalCommands.afterSetPageTarget || function(options){},
					generateNavigationMenu: mCustomGlobalCommands.generateNavigationMenu || function(parentId, serviceRegistry, commandRegistry,
					prefsService, searcher, handler, /* optional */editor){
				...
				afterGenerateBanner: mCustomGlobalCommands.afterGenerateBanner || function(parentId, serviceRegistry, commandRegistry,
				prefsService, searcher, handler, /* optional */editor){}
			};
			...
			function startProgressService(serviceRegistry) {
				...
				progressPane
						.setAttribute(
								"aria-label", messages['Operations - Press spacebar to show current operations']); //$NON-NLS-1$ //$NON-NLS-0$
				var progressService = serviceRegistry
						.getService("orion.page.progress"); //$NON-NLS-0$
				if (progressService) {
				...
				if (authService !== null) {
					authService.getUser().then(
							function(jsonData) {
								...
								if (jsonData.Name) {
									text = document
											.createTextNode(jsonData.Name);
								} else if (jsonData.login) {
									text = document
											.createTextNode(jsonData.login);
								}
								if (text) {
									if (node.childNodes.length > 0) {
										if (node.childNodes[0].nodeType === 3) {
											// replace original text
											node.replaceChild(text,
													node.childNodes[0]);
										} else {
			...
			function generateUserInfo(serviceRegistry, keyAssistFunction) {
				...
				var authServices = serviceRegistry
						.getServiceReferences("orion.core.auth"); //$NON-NLS-0$
				authenticationIds = [];
				...
				for ( var i = 0; i < authServices.length; i++) {
					...
					getLabel(authService, servicePtr)
							.then(
									function(label) {
										authService
												.getKey()
												.then(
														function(key) {
															...
															window
																	.addEventListener(
																			"storage", function(e) { //$NON-NLS-0$
																				if (authRendered[key] === localStorage
																						.getItem(key)) {
																					return;
																				}

																				authRendered[key] = localStorage
																						.getItem(key);
			...
			// Related links menu management. The related menu is reused as
			// content changes. If the menu becomes
			 ...
			 * @function
			 */
			function generateRelatedLinks(serviceRegistry, item, exclusions,
					commandRegistry, alternateItem) {
				...
				function getContentTypes() {
					...
					var contentTypeService = serviceRegistry
							.getService("orion.core.contenttypes"); //$NON-NLS-0$
					// TODO Shouldn't really be making service selection
					// decisions at this level. See bug 337740
					if (!contentTypeService) {
						contentTypeService = new mContentTypes.ContentTypeService(
								serviceRegistry);
						contentTypeService = serviceRegistry
								.getService("orion.core.contenttypes"); //$NON-NLS-0$
					}
					return contentTypeService.getContentTypes().then(
							function(ct) {
				...
				var contributedLinks = serviceRegistry
						.getServiceReferences("orion.page.link.related"); //$NON-NLS-0$
						...
						.when(
								...
								function() {
									...
									for ( var i = 0; i < contributedLinks.length; i++) {
										var info = {};
										var j;
										var propertyNames = contributedLinks[i]
												.getPropertyKeys();
										for (j = 0; j < propertyNames.length; j++) {
											info[propertyNames[j]] = contributedLinks[i]
													.getProperty(propertyNames[j]);
										}
										if (info.id) {
											function enhanceCommand(command) {
												if (command) {
													if (!command.visibleWhen
															|| command
																	.visibleWhen(item)) {
														var invocation = new mCommands.CommandInvocation(
																item, item,
																null, command,
																commandRegistry);
														customGlobalCommands.addRelatedLinkCommand(
																command,
																invocation);
																		...
																		function(
																			...
																			if (newItem
																				...
																				if (!command.visibleWhen
																					...
																					customGlobalCommands.addRelatedLinkCommand(
																							command,
																							new mCommands.CommandInvocation(
																									newItem,
																									newItem,
																									null,
																									command,
											...
											var command = null;
											var deferred = null;
											// exclude anything in the list of
											// exclusions
											var position = exclusions
													.indexOf(info.id);
											if (position < 0) {
												// First see if we have a
												// uriTemplate and name, which
												// is enough to build a command
												// internally.
												if (((info.nls && info.nameKey) || info.name)
														&& info.uriTemplate) {
													deferred = mExtensionCommands
															._createCommandOptions(
																	info,
																	contributedLinks[i],
																	serviceRegistry,
																	contentTypesCache,
												...
												// If we couldn't compose one,
												// see if one is already
												// registered.
												if (!command) {
												...
												// If it's not registered look
												// for it in
												// orion.navigate.command and
												// create it
												if (!command) {
													var commandsReferences = serviceRegistry
															.getServiceReferences("orion.navigate.command"); //$NON-NLS-0$
													for (j = 0; j < commandsReferences.length; j++) {
														var id = commandsReferences[j]
																.getProperty("id"); //$NON-NLS-0$
														if (id === info.id) {
															var navInfo = {};
															propertyNames = commandsReferences[j]
																	.getPropertyKeys();
															for ( var k = 0; k < propertyNames.length; k++) {
																navInfo[propertyNames[k]] = commandsReferences[j]
																		.getProperty(propertyNames[k]);
															}
															deferred = mExtensionCommands
																	._createCommandOptions(
																			navInfo,
																			commandsReferences[j],
																			serviceRegistry,
																			contentTypesCache,
																			true);
															deferreds
			...
			function renderGlobalCommands(commandRegistry) {
				...
				if (globalTools) {
					...
					commandRegistry.renderCommands(globalTools.id, globalTools,
							{}, {}, "tool"); //$NON-NLS-0$
				}
			}

			/**
			 * Support for establishing a page item associated with global
			 * commands and related links
			 */
			...
			/**
			 * Set a dirty indicator for the page. An in-page indicator will
			 * always be set. If the document has a title (set via
			 * setPageTarget), then the title will also be updated with a dirty
			 * indicator.
			 */
			...
			/**
			 * Set the target of the page so that common infrastructure
			 * (breadcrumbs, related menu, etc.) can be added for the page.
			 * 
			 * @param {Object}
			 *            options The target options object.
			 * @param {String}
			 *            options.task the name of the user task that the page
			 *            represents.
			 * @param {Object}
			 *            options.target the metadata describing the page
			 *            resource target. Optional.
			 * @param {String}
			 *            options.name the name of the resource that is showing
			 *            on the page. Optional. If a target parameter is
			 *            supplied, the target metadata name will be used if a
			 *            name is not specified in the options.
			 * @param {String}
			 *            options.title the title to be used for the page.
			 *            Optional. If not specified, a title will be
			 *            constructed using the task and/or name.
			 * @param {String}
			 *            options.breadcrumbRootName the name used for the
			 *            breadcrumb root. Optional. If not specified, the
			 *            breadcrumbTarget, fileService, task, and name will be
			 *            consulted to form a root name.
			 * @param {Object}
			 *            options.breadcrumbTarget the metadata used for the
			 *            breadcrumb target. Optional. If not specified,
			 *            options.target is used as the breadcrumb target.
			 * @param {Function}
			 *            options.makeAlternate a function that can supply
			 *            alternate metadata for the related pages menu if the
			 *            target does not validate against a contribution.
			 *            Optional.
			 * @param {Function}
			 *            options.makeBreadcrumbLink a function that will supply
			 *            a breadcrumb link based on a location shown in a
			 *            breadcrumb. Optional. If not specified, and if a
			 *            target is specified, the breadcrumb link will refer to
			 *            the Navigator.
			 * @param {Object}
			 *            options.serviceRegistry the registry to use for
			 *            obtaining any unspecified services. Optional. If not
			 *            specified, then any banner elements requiring Orion
			 *            services will not be provided.
			 * @param {Object}
			 *            options.commandService the commandService used for
			 *            accessing related page commands. Optional. If not
			 *            specified, a related page menu will not be shown.
			 * @param {Object}
			 *            options.searchService the searchService used for
			 *            scoping the searchbox. Optional. If not specified, the
			 *            searchbox will not be scoped.
			 * @param {Object}
			 *            options.fileService the fileService used for
			 *            retrieving additional metadata and managing the
			 *            breadcrumb for multiple file services. If not
			 *            specified, there may be reduced support for multiple
				...
				if (options.target) { // we have metadata
					if (options.searchService) {
						options.searchService
								.setLocationByMetaData(options.target); //$NON-NLS-0$
					}
					if (options.fileService && !options.breadcrumbTarget) {
						...
						fileSystemRootName = fileSystemRootName
								+ options.fileService
										.fileServiceName(options.target.Location);
					...
					generateRelatedLinks(options.serviceRegistry,
							options.target, exclusions, options.commandService,
							options.makeAlternate);
				} else {
					if (!options.breadcrumbTarget) {
						breadcrumbRootName = breadcrumbRootName || options.task
								|| options.name;
					}
					name = options.name;
					generateRelatedLinks(options.serviceRegistry, {
						NoTarget : ""
					}, exclusions, options.commandService,
							options.makeAlternate);
				}
				title = options.title;
				if (!title) {
			...
			function getToolbarElements(toolNode) {
				...
				var node = toolNode;
				// the trickiest part is finding where to start looking (section
				// or main toolbar).
				...
				if (toolbarNode) {
					elements.slideContainer = lib.$(
							".slideParameters", toolbarNode); //$NON-NLS-0$
					elements.parameterArea = lib.$(".parameters", toolbarNode); //$NON-NLS-0$
					elements.dismissArea = lib.$(
							".parametersDismiss", toolbarNode); //$NON-NLS-0$
					elements.notifications = lib.$(
							"#notificationArea", toolbarNode); //$NON-NLS-0$
					if (toolbarNode.parentNode) {
						elements.toolbarTarget = lib.$(
								".toolbarTarget", toolbarNode.parentNode); //$NON-NLS-0$
						if (elements.toolbarTarget) {
							var bounds = lib.bounds(toolbarNode);
							// assumes that there is never anything besides
							// notifications and slideout between toolbar and
							// its target
							// see
							// https://bugs.eclipse.org/bugs/show_bug.cgi?id=391596
				...
				if (elements.toolbarTarget && elements.toolbarTargetY) {
					...
					if (elements.notifications
							&& elements.notifications.classList
									.contains("slideContainerActive")) { //$NON-NLS-0$
						bounds = lib.bounds(elements.notifications);
						heightExtras += bounds.height;
					}
					if (elements.slideContainer
							&& elements.slideContainer.classList
									.contains("slideContainerActive")) { //$NON-NLS-0$
					...
					elements.toolbarTarget.style.top = elements.toolbarTargetY
							+ heightExtras + "px"; //$NON-NLS-0$
					elements.toolbarTarget.style.bottom = 0;
				}
			}

			/*
			 * This function adds a settings dialog to a page. It adds it so
			 * that a settings gear will appear at the right hand side
			 */
			...
			function addSettings(settings) {
				...
				navDropDown.updateContent = settings.updateContent
						.bind(settings);
			 ...
			 * @function
			 */
			function generateBanner(parentId, serviceRegistry, commandRegistry,
					prefsService, searcher, handler, /* optional */editor) {
				new mThemePreferences.ThemePreferences(prefsService,
						new mThemeData.ThemeData()).apply();

				var parent = lib.node(parentId);

				if (!parent) {
					throw messages["could not find banner parent, id was "]
							+ parentId;
				}
				// place the HTML fragment for the header.
				var range = document.createRange();
				range.selectNode(parent);
				var headerFragment = range
						.createContextualFragment(BannerTemplate);
				...
				// TODO not entirely happy with this. Dynamic behavior that
				// couldn't be in the html template, maybe it could be
				// dynamically
				...
				if (toolbar) {
					...
					toolbar.innerHTML = ToolbarTemplate
							+ commonHTML.slideoutHTMLFragment("mainToolbar"); //$NON-NLS-0$
				}
				var closeNotification = lib.node("closeNotifications"); //$NON-NLS-0$
				if (closeNotification) {
					closeNotification.setAttribute(
							"aria-label", messages['Close notification']); //$NON-NLS-1$ //$NON-NLS-0$
				}
				...
				// Set up a custom parameter collector that slides out of
				// adjacent tool areas.
				commandRegistry
						.setParameterCollector(new mParameterCollectors.CommandParameterCollector(
								getToolbarElements, layoutToolbarElements));

				document.addEventListener("keydown", function(e) { //$NON-NLS-0$
					if (e.keyCode === lib.KEY.ESCAPE) {
						var statusService = serviceRegistry
								.getService("orion.page.message"); //$NON-NLS-0$
						if (statusService) {
				...
				if (primaryNav) {
					PageLinks.createPageLinks(serviceRegistry,
							"orion.page.link").then(function(links) { //$NON-NLS-0$
				...
				// hook up split behavior - the splitter widget and the
				// associated global command/key bindings.
				...
				if (splitNode) {
					...
					if (side && main) {
						...
						// For any page that defines the splitter style as
						// "left:0%", we treate it as a collasped left pane as
						// default.
						...
						commandRegistry
								.registerCommandContribution(
										"pageActions", "orion.toggleSidePane", 1, null, true, new KeyBinding.KeyBinding('o', true)); //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-0$

						// editor behavior if needed
						if (editor) {
							mainSplitter.splitter.addResizeListener(function(
									node) {
								if (editor && node === main) {
				...
				// Assemble global commands, those that could be available from
				// any page due to header content or common key bindings.
				// make favorite
				var favoriteCommand = new mCommands.Command(
						{
							...
							callback : function(data) {
								var items = Array.isArray(data.items) ? data.items
										: [ data.items ];
								var favService = serviceRegistry
										.getService("orion.core.favorite"); //$NON-NLS-0$
								var progress = serviceRegistry
										.getService("orion.page.progress"); //$NON-NLS-0$
								var doAdd = function(item) {
									return function(result) {
										if (!result) {
											progress.progress(favService
													.makeFavorites(item),
													"Making favorite "
															+ item.Name);
										} else {
											serviceRegistry
													.getService(
															"orion.page.message").setMessage(item.Name + messages[' is already a favorite.'], 2000); //$NON-NLS-0$
										}
									};
								};
								for ( var i = 0; i < items.length; i++) {
									...
									progress
											.progress(
													favService
															.hasFavorite(item.ChildrenLocation
																	|| item.Location),
													"Checking favorite "
															+ item.Name).then(
													doAdd(item));
				...
				var openResourceDialog = function(searcher, serviceRegistry, /* optional */
						editor) {
					var favoriteService = serviceRegistry
							.getService("orion.core.favorite"); //$NON-NLS-0$
					var progress = serviceRegistry
							.getService("orion.page.progress"); //$NON-NLS-0$
					// TODO Shouldn't really be making service selection
					...
					if (!favoriteService) {
						...
						// service must be accessed via the registry so we get
						// async behaviour
						favoriteService = serviceRegistry
								.getService("orion.core.favorite"); //$NON-NLS-0$
					}
				...
				commandRegistry
						.registerCommandContribution(
								"globalActions", "eclipse.openResource", 100, null, true, new KeyBinding.KeyBinding('f', true, true)); //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-0$
				...
				var globalSearchCommand = new mCommands.Command({
					...
					callback : function(data) {
						...
						if (editor) {
							...
							if (selection.end > selection.start) {// If there
																	// is
																	// selection
																	// from
																	// editor,
								...
								searchString = model.getText(selection.start,
										selection.end);
							}
						}
				...
				commandRegistry
						.registerCommandContribution(
								"globalActions", "eclipse.globalSearch", 101, null, true, new KeyBinding.KeyBinding('h', true, false, true)); //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-0$
				...
				var toggleBanner = new mCommands.Command(
						{
							...
							callback : function() {
								...
								if (header.style.display === "none") { //$NON-NLS-0$
									...
									content.classList
											.remove("content-fixedHeight-maximized"); //$NON-NLS-0$
								} else {
									header.style.display = "none"; //$NON-NLS-0$
									footer.style.display = "none"; //$NON-NLS-0$
									content.classList
											.add("content-fixedHeight-maximized"); //$NON-NLS-0$
								}
								if (editor && editor.getTextView()) {
				...
				commandRegistry
						.registerCommandContribution(
								"globalActions", "orion.toggleTrim", 100, null, true, new KeyBinding.KeyBinding("m", true, true)); //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-0$
				...
				commandRegistry
						.registerCommandContribution(
								"globalActions", "orion.configDetailsPage", 100, null, true, new KeyBinding.KeyBinding(112, true, true)); //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-0$
				...
				commandRegistry
						.registerCommandContribution(
								"globalActions", "orion.backgroundOperations", 100, null, true, new KeyBinding.KeyBinding('o', true, true)); //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-0$
				...
				KeyAssistPanel.prototype = {
					create : function() {
						var keyAssistDiv = this._keyAssistDiv = document
								.createElement("div");//$NON-NLS-0$
						...
						var keyAssistInput = this._keyAssistInput = document
								.createElement("input"); //$NON-NLS-0$
						...
						var keyAssistContents = this._keyAssistContents = document
								.createElement("div");//$NON-NLS-0$
						keyAssistContents.classList.add("keyAssistContents");//$NON-NLS-0$
						keyAssistDiv.appendChild(keyAssistContents);
						var keyAssistTable = this._keyAssistTable = document
								.createElement('table'); //$NON-NLS-0$
						...
						keyAssistContents
								.addEventListener(
										util.isFirefox ? "DOMMouseScroll" : "mousewheel", function(e) { //$NON-NLS-1$ //$NON-NLS-0$
											this._scrollWheel(e);
										}.bind(this));
						document.addEventListener("keydown", function(e) { //$NON-NLS-0$
							if (e.keyCode === lib.KEY.ESCAPE) {
					...
					createContents : function() {
						...
						if (editor && editor.getTextView()) {
							...
							editorActions.sort(function(a, b) {
								return textView.getActionDescription(a).name
										.localeCompare(textView
												.getActionDescription(b).name);
							});
							...
							for ( var i = 0; i < editorActions.length; i++) {
								...
								var actionDescription = textView
										.getActionDescription(actionID);
								var bindings = textView
										.getKeyBindings(actionID);
								for ( var j = 0; j < bindings.length; j++) {
									var bindingString = mUIUtils
											.getUserKeyString(bindings[j]);
									this.createItem(bindingString,
					...
					createItem : function(bindingString, name, execute) {
						if (this._filterString) {
							...
							if (insensitive(name).indexOf(s) === -1
						...
						column.appendChild(document
								.createTextNode(bindingString));
					},
					...
					hide : function() {
						...
						var hasFocus = keyAssistDiv === activeElement
								|| (keyAssistDiv
										.compareDocumentPosition(activeElement) & 16) !== 0;
						keyAssistDiv.style.display = "none"; //$NON-NLS-0$
						if (hasFocus
								&& document
										.compareDocumentPosition(this._previousActiveElement) !== 1) {
					...
					select : function(forward) {
						var rows = this._keyAssistTable
								.querySelectorAll(".keyAssistItem"), row; //$NON-NLS-0$
						if (rows.length === 0) {
							this._selectedIndex = -1;
							return;
						}
						...
						if (this._selectedIndex !== -1) {
							...
							this._keyAssistTable.setAttribute(
									"aria-activedescendant", row.id); //$NON-NLS-0$
					...
					show : function() {
						...
						this.createContents();
						this._keyAssistContents.style.height = Math
								.floor(this._keyAssistDiv.parentNode.clientHeight * 0.8)
								+ "px"; //$NON-NLS-0$
				...
				commandRegistry
						.registerCommandContribution(
								"globalActions", "eclipse.keyAssist", 100, null, true, new KeyBinding.KeyBinding(191, false, true)); //$NON-NLS-1$ //$NON-NLS-0$
				var addKeyAssistAction = function() {
					editor.getTextView().setKeyBinding(
							new KeyBinding.KeyBinding(191, false, true,
									!util.isMac, util.isMac),
							keyAssistCommand.id);
					editor.getTextView().setAction(keyAssistCommand.id,
							keyAssistCommand.callback, keyAssistCommand);
				};
				if (editor) {
					if (editor.getTextView()) {
						...
						editor.addEventListener(
								"TextViewInstalled", addKeyAssistAction); //$NON-NLS-0$
					}
				}
				...
				function readTargetPreference(serviceRegistry) {
					serviceRegistry
							.registerService(
									"orion.cm.managedservice", //$NON-NLS-0$
									{
										updated : function(properties) {
											var target;
											if (properties
													&& properties["links.newtab"] !== "undefined") { //$NON-NLS-1$ //$NON-NLS-0$
