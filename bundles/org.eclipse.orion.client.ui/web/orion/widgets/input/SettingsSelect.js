...
define(['orion/objects', 'orion/webui/littlelib', 'orion/widgets/input/Select'],  function(objects, lib, Select) {
	...
	function SettingsSelect( params, node ){
 	...
 	objects.mixin(SettingsSelect.prototype, Select.prototype, makeSelect());

	function makeLocalSelect(params) {
		var newSelect = makeSelect();

		if (params.local) {
			newSelect.templateString =
			'<label>' + //$NON-NLS-0$
				'<span class="setting-label"></span>' + //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-0$
				'<select class="setting-control"></select>' + //$NON-NLS-0$
			'</label>';  //$NON-NLS-0$

			objects.mixin(SettingsSelect.prototype, Select.prototype, newSelect);
		}
	}
