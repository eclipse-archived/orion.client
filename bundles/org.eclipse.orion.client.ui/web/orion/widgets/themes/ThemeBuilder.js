...
function(messages, mCommands, mCommandRegistry, lib, mSetup, colors, util, jsExample, htmlExample, cssExample, javaExample) {
	...
	var fontList = ["Source Code Pro", "Andale Mono", "Arial", "Consolas", "Courier New", "Helvetica", "Helvetica Neue", "Monaco", "Verdana", "Vera Mono"];
		...
		for (var i = 0; i < scopeList.length; i++){
			if (scopeList[i].id === "editorThemeFontSize"){
				...
				fontList.forEach(function(element, index, array) {
					if (index === 0) {
						htmlString = htmlString + "<option value='" + element + "'  selected='selected'>" + element + "</option>";
					} else {
	...
	function returnAvailableFontsList(fontList) {
		...
		// fontList.forEach(function(element, index, array) {
		// 	if (element !== "Source Code Pro") { // We always want to have Source Code Pro in the list
		// 		if (monoWidth === calculateWidth(element + ',monospace') || sansWidth === calculateWidth(element + ',sans-serif') || serifWidth === calculateWidth(element + ',serif')) {
