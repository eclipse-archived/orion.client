...
define ([
	...
	function HoverImpl(editor, inputManager, serviceRegistry) {
		this.editor = editor;
	...
	HoverImpl.prototype = {
		computeHoverInfo: function (offset){
			var hoverInfo = [];
			this._applicableProviders.forEach(function(provider){
				...
				if (providerImpl.computeHoverInfo) {
					...
					var promise = providerImpl.computeHoverInfo(EditorContext.getEditorContext(this.serviceRegistry), context);
					hoverInfo.push({title: provider._properties.tipTitle, promise: promise});
				}
			}.bind(this));
	...
	HoverFactory.prototype = {
		createHover: function(editor) {
			return new HoverImpl(editor, this.inputManager, this.serviceRegistry);
		}
	};
