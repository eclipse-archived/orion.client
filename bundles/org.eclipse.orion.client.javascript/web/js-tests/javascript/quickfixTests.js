...
define([
	...
	describe('Quick Fix Tests', function() {
		...
		function setup(options) {
			...
			var props = {};
			props[rule.id] = rule.severity;
			validator.updated(props);
	...
	//NO-EMPTY-BLOCK
		it("Test no-empty-block-1", function() {
		    var rule = createTestRule('validate_no_empty_block');
	...
	//NO-EXTRA-SEMI
		it("Test no-extra-semi-1", function() {
		    var rule = createTestRule('validate_no_extra_semi');
		    return getFixes({buffer: 'function f() {};', 
		                      rule: rule,
	...
	//NO-FALLTHROUGH
		it("Test no-fallthrough-1", function() {
		    var rule = createTestRule('validate_no_fallthrough');
	...
	//NO-UNDEF
		it("Test no-undef-defined-1", function() {
		    var rule = createTestRule('validate_no_undef');
