					...
					that.ternworker.postMessage(
						...
						function(type, err) {
							if(err) {
								...
								expected.done = 0;
								expected.result = [];
								var searchParams = {keyword: node.name, resource: that.scriptresolver.getSearchLocation(), fileNamePatterns:["*.js"], caseSensitive: true, incremental:true, shape: 'group' }; //$NON-NLS-1$
